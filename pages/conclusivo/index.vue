...
// CONCLUSIVO
  ...
  const taxa_autossuficiencia = ref(0) // VARIAVEL PARA TAXA DE AUTOSSUFICIÊNCIA DO MES
  ...
  const abrirDialog = () =>{ 
      ...
      taxa_autossuficiencia.value = calcularTaxaAutossuficiencia(Number(ano), Number(mes))
  ...
  const buscarConsumoMedio = () => {
   ...
   /* CÁLCULO SOMA ESPECIAL PARA AUTOSSUFICIÊNCIA TIRANDO O HOSPITAL E O UPA*/
  const somarConsumoPorMesAnoEspecial = (ano, mes) => { 
    ...
    const soma = filtrados.reduce((acc, item) => {
      if (UCsEspeciais.includes(String(item.uc))) {
        // Procura a geração da UC na lista de geracoes
        const geracaoUC = geracoes.value.find(g => g.uc === item.uc && g.ano === ano && g.mes === mes)
  ...
  const calcularTaxaAutossuficiencia = (ano, mes) => {

    const consumoMedioUsinas = buscarConsumoMedio()
    const consumoMedioUnidades = somarConsumoPorMesAnoEspecial(ano, mes)

    const totalCompensado = energia_compensada.value + (geracao_total.value - energia_injetada.value)
    const totalConsumo = consumoMedioUnidades + consumoMedioUsinas

    const taxaAutossuficiencia = (totalCompensado / totalConsumo) * 100


    // console.log("CONSUMO MEDIO USINAS: ", consumoMedioUsinas)
    // console.log("CONSUMO MEDIO UNIDADES: ", consumoMedioUnidades)

    // console.log("TOTAL COMPENSADO: ", totalCompensado)
  ...
  <!-- <v-dialog v-model="dialogRelatorioPDF" width="1024">
    <v-card>
      <v-card-title style="background: linear-gradient(to bottom, #4d7fff, #1e73be); color: white;">
        <span class="text-h5">RELATÓRIO CONCLUSIVO - DESEMPENHO ENERGÉTICO</span>
      </v-card-title>
      ...
      <v-card-text ref="tabelaElement" id="OSPDF">
        <v-row>
          ...
          <v-col cols="8" class="text-center"> 
            <h5>Sistema de Monitoramento Energético</h5>
            <h6>Período: {{ periodo_mes }}/{{ periodo_ano }}</h6>
          </v-col>
          <v-col cols="2">
        ...
        <h4>1. Sumário Executivo</h4>
        <v-table density="compact">
          <tbody>
            <tr>
              <td><b>Projeção total no mês</b></td>
              <td>{{ projecao_total }} kWh</td>
            </tr>
            <tr>
              <td><b>Geração total no mês</b></td>
              <td>{{ geracao_total }} kWh</td>
            </tr>
            <tr>
              <td><b>Consumo total no mês</b></td>
              <td>{{ consumo_total }} kWh</td>
            </tr>
            <tr>
              <td><b>Saldo líquido de energia</b></td>
              <td>{{ saldo_liquido }} kWh</td>
            </tr>
            <tr>
              <td><b>Economia total estimada</b></td>
              <td>R$ {{ economia_total }}</td>
            </tr>
            <tr>
              <td><b>CO₂ evitado</b></td>
              <td>{{ co2_evitado }} t (equivalente a {{ arvores_equivalente }} árvores)</td>
            </tr>
          </tbody>
        </v-table>
        ...
        <h4>2. Indicadores Globais</h4>
        <v-table density="compact">
          <tbody>
            <tr>
              <td>Energia Compensada</td>
              <td>{{ energia_compensada }} kWh</td>
            </tr>
            <tr>
              <td>Energia Injetada</td>
              <td>{{ energia_injetada }} kWh</td>
            </tr>
            <tr>
              <td>Taxa de Autossuficiência</td>
              <td>{{ taxa_autossuficiencia }}%</td>
            </tr>
            <tr>
              <td>AutoConsumo Evitado Total</td>
              <td>R$ {{ custo_autoconsumo_total }}</td>
            </tr>
            <tr>
              <td>Custo Evitado Total (unidades + usinas)</td>
              <td>R$ {{ custo_evitado_total }}</td>
            </tr>
          </tbody>
        </v-table>
        ...
        <h4>3. Rankings do Mês</h4>
        <p>🏆 Top 5 – Mais Receberam Energia Compensada</p>

        <ul>
          <li v-for="(item, index) in ranking_mais_compensacao" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – {{ Number(item.enerInjTUSD).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }} kWh
          </li>
        </ul>


        <p>🏆 Top 5 – Mais Consumiram Energia</p>
        <ul>
          <li v-for="(item, index) in ranking_mais_consumo" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – {{ Number(item.consumokWh).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }} kWh
          </li>
        </ul>

        <p>🏆 Top 5 – Mais Economizaram (R$)</p>
        <ul>
          <li v-for="(item, index) in ranking_mais_economia" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – R$ {{ Number(item.energiaInjetada).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }}
          </li>
        </ul> 

        <p>🏆 Top 5 – Maior Geração de Energia (kWh)</p>
        <ul>
          <li v-for="(item, index) in ranking_mais_geracao" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – {{ Number(item.geracao).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }} kWh
          </li>
        </ul>

        <p>🏆 Top 5 – Maior Saldo de Energia</p>
        <ul>
          <li v-for="(item, index) in ranking_mais_saldo" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – {{ Number(item.saldoEnergia).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }} kWh
          </li>
        </ul>

        <p>⚠️ Top 5 – Maior Aumento de Consumo</p>
        <ul>
          <li v-for="(item, index) in ranking_maior_aumento_consumo" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – {{ Number(item.aumento_kWh).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }} kWh
          </li>
        </ul>

        <p>✅ Top 5 – Maior Redução de Consumo</p>
        <ul>
          <li v-for="(item, index) in ranking_maior_aumento_consumo" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – {{ Number(item.aumento_kWh).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }} kWh
          </li>
        ...
        <p>✅ Top 5 – Maior Redução de Consumo</p>
        <ul>
          <li v-for="(item, index) in ranking_maior_reducao_consumo" :key="item.uc">
            {{ index + 1 }}º - {{ item.nome }} (UC: {{ item.uc }}) – {{ Number(item.reducao_kWh).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }} kWh
          </li>
        </ul>

        <v-divider class="my-4"></v-divider> 

        <h4>4. Eficiência Econômica</h4>
        <v-table density="compact">
          <tbody>
            <tr>
              <td>Economia Até Hoje</td>
              <td>R$ {{ economia_ate_hoje }}</td>
            </tr>
            <tr>
              <td>Economia Do Ano</td>
              <td>R$ {{ economia_do_ano }}</td>
            </tr>
            <tr>
              <td>Payback Acumulado</td>
              <td>{{ payback_acumulado }}%</td>
            </tr>
          </tbody>
        </v-table>
        ...
        <h4>5. Indicadores Ambientais</h4>
        <v-table density="compact">
          <tbody>
            <tr>
              <td>CO₂ Evitado no Mês</td>
              <td>{{ co2_evitado }} t</td>
            </tr>
            <tr>
              <td>CO₂ Evitado no Geral</td>
              <td>{{ co2_evitado_acumulado }} t</td>
            </tr>
            <tr>
              <td>Equivalente em Árvores</td>
              <td>{{ arvores_equivalente }}</td>
            </tr>
          </tbody>
        </v-table>
        ...
        <h4>6. Resumo por Usina</h4>
        <div v-for="usina in usinas_desempenho" :key="usina.id" style="margin-bottom: 20px;">
          <strong>{{ usina.nome }}</strong><br>
          Geração no mês: {{ usina.geracao }} kWh ({{ usina.desempenho }}% da meta)<br>  
        </div>

        <v-divider class="my-4"></v-divider>

        <h4>7. Projeção para o Próximo Mês</h4>
        <p>Geração esperada: {{ projecao_geracao }} kWh</p>
         <p><i>{{ comentario_sazonalidade }}</i></p> 
        ...
        <v-col cols="12" class="text-center">
            <v-table>
                 <tbody>
                  ...
                  <tr>
                    <td style="height: 35px;">Taxa de Autossuficiência</td>
                    <td style="height: 35px;">{{ taxa_autossuficiencia }}%</td>
                  </tr>
                  <tr>
